...
class InMemoryRepoFactory:
    ...
    async def create_details_repo(self, entity: str, key: int) -> InMemoryDetailsRepo:
        repo = InMemoryDetailsRepo(entity, key)
        repo._db = self._database
        return repo

    async def create_list_repo(self, entity: str, filters: Map | None = None) -> InMemoryListRepo:
